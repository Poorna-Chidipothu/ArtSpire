...
// import { Blurhash } from "react-blurhash"; // Uncomment if using Blurhash
...
const Gallery = () => {
  const { url, searchQuery, setSearchQuery, setImageShow, setShownImageData } = useContext(StoreContext);
  ...
  const fetchMedia = async (page, query = "", filterType = 'all') => {
    try {
      const response = await axios.get(`${url}/api/get-media/get-all-media`, {
        params: { page, limit: 20, search: query },
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      });

      const newMedia = response.data.media.map(media => ({
        ...media,
        displayType: media.type === 'video' ? 'video' : 'image'
      }));

      setSomeImages(prev => {
        const combinedMedia = page === 1 ? newMedia : [...prev, ...newMedia];

        // If filtering is needed, apply it here
        const filteredResult = filterType === 'photos'
          ? combinedMedia.filter(m => m.displayType === 'image')
          : filterType === 'videos'
            ? combinedMedia.filter(m => m.displayType === 'video')
            : combinedMedia;
  ...
  const fetchAiImages = async (page) => {
    // setIsLoading(true);
    try {
      const response = await axios.get(`${url}/api/get-media/get-all-aigen-images`, {
        params: { page, limit: 20 },
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      });

      const newAiImages = response.data.images.map(image => ({
        ...image,
        displayType: 'ai-image',
        url: image.aigen_picture_url
      }));

      setSomeImages(prev => {
        const combinedImages = page === 1 ? newAiImages : [...prev, ...newAiImages];
        return combinedImages;
      });

      if (newAiImages.length < 20) {
        setHasMore(false);
      }
    } catch (error) {
  ...
  const handleFilterChange = (newFilter) => {
    ...
    switch (newFilter) {
      case 'ai-gallery':
        ...
        break;
      case 'photos':
        fetchMedia(1, searchQuery, 'photos');
        break;
      case 'videos':
        fetchMedia(1, searchQuery, 'videos');
        break;
      default:
  ...
  const handleLike = async (mediaId, isLiked) => {
    try {
      const response = await axios.post(`${url}/api/like`, { mediaId }, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      });
  ...
  const handleImageClick = (clickedImage, index) => {
    ...
    const endIdx = Math.min(filteredImages.length, index + 6);
    const surroundingImages = filteredImages.slice(startIdx, endIdx);

    setShownImageData({
      currentImage: clickedImage,
      surroundingImages: surroundingImages,
      currentIndex: index - startIdx,
    });
    setImageShow(true);
  };
  ...
  useEffect(() => {
    if (filter === 'all') {
      setFilteredImages(someImages);
    } else if (filter === 'ai-gallery') {
      setFilteredImages(someImages.filter(img => img.displayType === 'ai-image'));
    } else if (filter === 'photos') {
      setFilteredImages(someImages.filter(img => img.displayType === 'image'));
    } else if (filter === 'videos') {
      setFilteredImages(someImages.filter(img => img.displayType === 'video'));
    }
  }, [someImages, filter]);
    ...
    if (page > 1) {
      switch (filter) {
        case 'ai-gallery':
          ...
          break;
        case 'photos':
          fetchMedia(page, searchQuery, 'photos');
          break;
        case 'videos':
          fetchMedia(page, searchQuery, 'videos');
          break;
        default:
  ...
  return (
    <section className="gallery">
      <div className="filter_container">
        ...
        <span className={`all ${filter === 'all' ? 'active' : ''}`} onClick={() => handleFilterChange('all')}>All</span>
        <span className={`photos ${filter === 'photos' ? 'active' : ''}`} onClick={() => handleFilterChange('photos')}>Photos</span>
        <span className={`videos ${filter === 'videos' ? 'active' : ''}`} onClick={() => handleFilterChange('videos')}>Video</span>
        <span className={`ai-gallery ${filter === 'ai-gallery' ? 'active' : ''}`} onClick={() => handleFilterChange('ai-gallery')}>AI-Gallery</span>
      </div>
      ...
      {filteredImages.length > 0 ? (
        <InfiniteScroll
          ...
          endMessage={
            <p style={{ textAlign: "center", fontSize: "1.5rem", color: "#5db62c" }}>
        ...
        >
          ...
          >
            {filteredImages.map((media, index) => (
              <div key={index} className="media-container" onMouseLeave={media.type === "video" ? () => handleVideoPause(index) : undefined}>
                {/* If you're using BlurHash, uncomment the code below */}
                {/* {!loadedImages[index] && media.blurHash && (
                  <Blurhash
                    hash={media.blurHash}
                    width="100%"
                ...
                {
                  media.displayType === 'ai-image' ? (
                    <div className="ai_image_card">
                      <img
                        src={media.url}
                        alt="AI-Img"
                        ...
                        <>
                          <span className="img_like" title="Like Image">
                            <ion-icon
                              ...
                              onClick={() => handleLike(media._id, media.isLiked)}
                              style={{ color: media.isLiked ? "red" : "black" }}
                            ></ion-icon>
                          </span>

                          <div className="media_content">
                            <span>
                              <div className="img_holder">{media.uploader[0]?.toUpperCase()}</div>
                              <h3>{media.uploader}</h3>
                            </span>

                            <a
                              href={getDownloadUrl(media.url || '')}
                              download
                              className="media_dwn"
                              title="Download Media"
                            >
